    ...
    module Wrapper
      class Wrapper
        ...
        def add_message(message)
          copy_message(message, @redis.xadd(message.stream, message.payload, message.id))
        end
        ...
        def ack_message(group, message)
          @redis.ack(message.stream, group, message.id)
        end
        ...
        def copy_message(message, new_id)
          shallow_copy = message.dup
          shallow_copy.id = new_id
          shallow_copy
        end
      end
    end
